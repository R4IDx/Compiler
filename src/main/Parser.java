...
public class Parser {
    ...
    public ASTNode parse(List<Token> ts) {
        ...
        ASTNode result = parseAdditionSubtraction();

        // Überprüfe, ob alle öffnenden Klammern geschlossen wurden
        if (currentTokenIndex < tokens.size()) {
            Token token = tokens.get(currentTokenIndex);
            if (token.type.equals("SPECIAL") && token.value.equals("(")) {
                openParenthesesCount++;
            } else if (token.type.equals("SPECIAL") && token.value.equals(")")) {
                if (openParenthesesCount > 0) {
                    openParenthesesCount--;
                } else {
                    throw new IllegalArgumentException("Mismatched parentheses: More closing parentheses than opening parentheses");
                }
            }
        }

        // Überprüfe, ob alle schließenden Klammern geöffnet wurden
        if (openParenthesesCount > 0) {
            throw new IllegalArgumentException("Mismatched parentheses: More opening parentheses than closing parentheses");
        }
